...
6. 사용자는 별도 입력 없이 Task를 완료하고 즉시 완료 열에서 확인한다.
...
- **AC-5:** Task 완료 시 상태·완료 시각·focus 해제는 하나의 원자적 전이로 처리되며 새 완료 기록을 요구하지 않는다.
- **AC-6:** 완료 과정은 별도 모달이나 필수 입력 없이 끝나고 기존 외부 연결은 유지된다.
